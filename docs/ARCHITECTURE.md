...
> - (STEP 9) `PlanWeek.actions` = `{ id, domain, text }[]`, `checks` = `{ id, label }[]` (같은 id), `WeeklyCheckIn.completed` = `{ [checkId]: boolean }`. 주차별 최신 체크 1건을 사용하며, 메모(note)는 최소수집 원칙에 따라 입력받지 않음
...
### 5.6 12주 계획 (STEP 9 구현)

- `src/domain/plan/*` (순수 함수): TOP 3 영역 × 단계(1~4 기초 / 5~8 운동·식습관 / 9~12 유지·평가) 실천 라이브러리로 계획 조립. 단계 앞 2주는 영역당 1개, 뒤 2주는 2개로 점진적 확대. 1·8·12주에는 시작 기록·중간 점검·재분석 항목. 중복 실천 제거, 입력에 맞지 않는 항목(비흡연자의 금연 등) 교체.
- LLM은 **주차별 코칭 메시지만** 작성 (실천 항목 변경 불가), 안전 검사 통과한 주차만 반영.
